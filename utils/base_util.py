...
def timer(title=''):
    ...
    if title == '':
        title = str(ti.get_t0())
    get_logger('timer').info("|{}| - begin".format(title))
    yield ti
    get_logger('timer').info(
        "|{}| - done in {:.4f}s".format(title, ti.get_delay_t0()))
...
# 打log使用
def get_logger(logger_name='STD_LOG', filename=None,
               level=logging.DEBUG, file_level=logging.INFO,
               fstr='%(asctime)s - %(name)s - %(levelname)s - %(message)s           |-| %(filename)s-%(funcName)s-%(lineno)s'):
    global logger_dict

    if logger_dict and logger_name == 'STD_LOG':
        for key in logger_dict:
            if key != 'timer':
                return logger_dict[key]

    if filename is None:
        filename = BASE_DIR + logger_name + '.log'
    logger_name = logger_name[:20]

    if logger_name == 'timer':
        key = 'timer'
    else:
        key = md5(f'{filename}-{logger_name}-{level}-{file_level}-{fstr}')
